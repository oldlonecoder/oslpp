...
ansi_parser::ansi_parser(descriptor& _data): stream(_data)
{
    //...
}
...
rem::code ansi_parser::parse(event& ev)
{
    ...
    if (cursor == stream.end()) return rem::code::eof;
    log::debug() << "parsing sequence: {" << color::lightsteelblue3 <<  tux::string::bytes(stream.buffer()) << color::reset << '}' << log::eol;
    if (*cursor == 0x1b)
    {
    ...
    if((*cursor < 32) || (*cursor == 127))
    {
        ...
        log::write() << "KEY_COMMAND: " << color::yellow << (int)*cursor << color::reset << "';" << log::eol;
...
rem::code ansi_parser::parse_ESC(event& ev)
{
    ...
    {
        case 'P':
            log::debug() <<  static_cast<int>(*cursor) << rem::code::notimplemented << log::eol;
...
rem::code ansi_parser::parse_csi(event& ev)
{
    ...
    while (true) {
        ...
        log::write() << " Unterminated CSI: switchting to parse_ss1_2" << log::eol;
        return parse_ss1_2(ev);
    }
}
...
rem::code ansi_parser::parse_mouse(event& ev, std::vector<int>&& args)
{
    ...
    if(event::mouse_data.meta)
        log::info() << color::pair({.fg=color::grey100, .bg=color::red4}) << "meta" << log::eol;
...
rem::code ansi_parser::parse_ss1_2(event& ev)
{
    ...
    for(int x=2;x < ln; x++)
    {
        ...
        code = (code << 8) | _b_;
        log::write() << std::format("{:04x}",code) << log::eol;
        if(key_event tmp_kev = key_event::query(code); tmp_kev.mnemonic != key_event::None)
        {
            log::write() << "Match:" << color::lime << tmp_kev.description << log::eol;
            ev.data.kev = tmp_kev;
            ev.event_type = event::type::key_command;
            return rem::code::ready;
        }
